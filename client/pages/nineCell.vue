  ...
  <div>
    <div class="flex gap-12 p-10">
      <div class="w-3/4 h-[calc(100vh-80px)]">
          <ul class="flex w-full h-[25%]">
            <li class="text-[25vw] border rounded w-1/4 h-full flex justify-center items-center">
            ...
            <li v-for="randX in randXs" class="text-[12vw] pt-2 border rounded border-l-0 w-1/4 h-full flex justify-center items-center">
              <p>{{ randX }}</p>
            </li>
          </ul><!-- 横の問題マス -->
          <div class="flex w-full h-[75%]">
            <ul class="w-1/4 h-full">
              <li v-for="randY in randYs" class="text-[12vw] pt-2 border rounded border-t-0 w-full h-1/3 flex justify-center items-center">
                <p>{{ randY }}</p>
              </li>
            </ul><!-- 縦の問題マス -->
            <ul class="flex flex-wrap w-3/4 h-full">
              <li id="form" v-for="(form, indexF) in forms" v-bind:class="{formsBgc: formsColor[indexF]}" 
                class="border rounded border-t-0 border-l-0 w-1/3 h-1/3 flex justify-center items-center">
                <input type="checkbox" v-model="formsColor[indexF]" :value="formsColor" class="hidden">
                <p class="text-[12vw] pt-2" >{{ form }}</p>
              </li>
            </ul><!-- 回答マス -->
          </div>
      </div>
